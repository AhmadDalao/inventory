    ...
    return [
        ...
        [
            ...
            'features' => [
                ...
                'Looks up active items by item barcode, package barcode, SKU, item name, or assigned location.',
                'Shows item image, SKU, barcode, current quantity, stock value, and per-location balances.',
                'Usage and refill carts support count, volume, mass, length, area, and custom canonical units.',
                'Admin-defined package presets convert boxes, bottles, bags, or other packages into the item canonical unit.',
                'Proof requirements are enforced from global defaults and per-item overrides before the atomic batch posts.',
                'Every accepted line keeps its entered amount, package, converted amount, employee department, manager, and proof.',
            ...
            'steps' => [
                ...
                'Pick movement type, assigned storage, package, entered quantity, reason, and notes.',
                'Attach a proof image when the item or global policy requires it.',
                'Review the converted canonical amount and projected balance before submitting.',
            ...
            'rules' => [
                ...
                'Employees select admin-defined packages but cannot invent conversion rates.',
                'Balances always use the item canonical unit; packages remain audit/display metadata.',
        ...
        [
            ...
            'route' => '/handovers',
            'summary' => 'Handovers track temporary staff use, long-term staff custody, and accountable stock transfers between storage owners.',
            'features' => [
                'Storage owner/admin can create a handover to staff.',
                'Long-term custody keeps interchangeable inventory assigned to one employee for weeks or months without misclassifying it as a fixed Asset.',
                ...
                'Custody recipients can submit multiple partial return events while unresolved quantity remains assigned to them.',
                'Custody returns classify quantity as Serviceable, Damaged, Consumed/Worn Out, or Lost/Missing.',
                'Damaged custody returns require a protected proof image; lost quantity requires an explanation.',
                'Approved serviceable stock returns to the source, damaged stock enters hidden quarantine, consumed/lost stock is written off, and still-held stock remains in the handover buffer.',
                'Approved damaged/lost returns can create a linked replacement request without automatically issuing stock.',
                'Owner-only quarantine controls can return repaired stock to service or dispose/write it off with an audit reason.',
                'Closeout is returned-first: recipient enters how many came back and the system calculates used quantity.',
                'New temporary-use handovers use one operational reconciliation per unit instead of expected-usage forecasting or per-item reason guessing.',
                'Operational totals cover Online, Walk-in, Event, Sport, Damage, Complimentary, No Show, and Other.',
                'Operational used is Online minus No Show, plus Walk-in, Event, Sport, Damage, Complimentary, and Other.',
            ...
            'rules' => [
                'Temporary use records short-term consumption; long-term custody records employee-held inventory; storage transfer records relocation between storage balances.',
                'Operational usage belongs only to temporary staff handovers, not storage transfers or custody returns.',
                ...
                'No Show cannot exceed Online. A negative Difference is blocked; a positive Difference requires discrepancy evidence and an audited variance reason.',
                'Custody history is never hard-deleted. A custody handover closes only when no quantity remains held.',
                'Quarantined stock is excluded from active availability until an owner returns it to service.',
            ],
        ],
        [
            'slug' => 'wristband-api-audit',
            'title' => 'Wristband API Audit',
            'icon' => 'scan',
            'audience' => 'Owner, Operations, Storage Owners, Admin',
            'route' => '/wristbands',
            'summary' => 'Wristband codes and KONA check-ins provide hidden audit evidence for temporary-use handovers without becoming a second stock authority.',
            'features' => [
                'Import unique wristband codes from CSV/XLSX and map each code permanently to one eligible item/color.',
                'Code states are Available, Used, and Void; used codes remain in audit history instead of being deleted.',
                'Manual Only keeps the existing handover workflow; API Audit records distinct check-ins as evidence; Paused records exceptions without consuming codes.',
                'Global, per-storage, and per-session switches let the owner stop API checking immediately.',
                'Valid API scans never create an inventory movement or change a handover status.',
                'Paused events can be accepted selectively or discarded with an audited reason after review.',
                'Owner Final Review compares physical used, staff reporting, distinct API check-ins, paused periods, exceptions, and variance.',
                'Staff cannot see hidden API evidence before submitting their own report.',
                'Per-storage keys are hashed, rotatable, HTTPS-only, rate-limited, and optionally restricted by IP address.',
            ],
            'steps' => [
                'Enable external QR tracking on an active count-based wristband item.',
                'Import codes and confirm Available registry counts without changing physical stock.',
                'Configure and enable one authorized storage integration.',
                'Create a normal temporary-use handover and select API Audit when needed.',
                'Pause, resume, or switch the session permanently to Manual Only at any time.',
                'Let staff complete receipt, returns, and operational reconciliation normally.',
                'Review API evidence and acknowledge any variance before final handover approval.',
            ],
            'rules' => [
                'Manual reconciliation is always available and remains the stock authority.',
                'API Audit never deducts directly from source storage and never posts stock twice.',
                'Disabling or pausing API checking never cancels, alters, or closes the handover.',
                'Storage transfers and long-term custody do not use wristband API evidence.',
                'Only authorized owners can reverse accepted codes or control integrations.',
            ],
        ],
        [
            'slug' => 'purchases',
        ...
        [
            ...
            'features' => [
                ...
                'Assign editable business positions for Operations, Cleaning, Stores, Maintenance, Finance, IT, Guest Services, Beach Operations, Office Administration, and general staff.',
                'Positions & Permissions manages each template label, default access level, department, and exact permission checklist.',
                'Applying a position copies its current defaults to the user.',
                ...
                'Open Team Hierarchy to search and filter employees, inspect who manages and is managed by each user, assign one manager to multiple selected staff, or use the optional compact tree.',
                'From Edit User, open Team & Reporting to change the manager, inspect or remove direct reports, and search/select more employees for that manager.',
            ...
            'steps' => [
                'Create or review reusable templates under Positions & Permissions.',
                'Create user with name, email, and position; the template supplies the recommended access, department, and permissions.',
                'Choose the direct manager and assign the storages the employee is allowed to see.',
                'Use Member for ordinary storage access and Owner only when the employee may approve stock for that storage.',
                'Apply the position template again only when you intentionally want to replace custom access defaults.',
            ...
            'rules' => [
                ...
                'Editing or archiving a position never changes users who already received it.',
                'Position templates never assign storage ownership, manager routing, or mobile capabilities.',
                'Manager assignment grants observer visibility and notifications; it does not grant stock approval by itself.',
                'Storage approval requires co-owner access to that storage or global Owner access.',
                'Permissions do not bypass storage assignment; unassigned item and storage records stay unavailable.',
                'The global Owner can use audited Owner Resolution actions to recover, reopen, safely close, or void workflows without editing the database.',
        ...
        [
            ...
            'features' => [
                ...
                'Storage assignments carry a Member or Owner role; only the Owner role grants location approval authority.',
                'Each accepted mobile stock action records the employee manager and notifies the manager and global Owner.',
                ...
                'Usage and Refill Storage carts support decimal measured quantities and show the canonical conversion before submission.',
                'Usage/refill proof is enforced by global defaults plus each item Inherit, Required, or Optional policy.',
                'Accepted movements snapshot employee department and direct manager for stable historical reporting.',
            ...
            'rules' => [
                ...
                'A manager can observe a direct report activity but cannot approve stock unless also assigned as a storage co-owner.',
                ...
                'Flutter previews conversions, but PHP remains the only conversion and stock authority.',
            ],
        ],
        [
            'slug' => 'departments-measured-reporting',
            'title' => 'Departments And Measured Reporting',
            'icon' => 'reports',
            'audience' => 'Owner, Admin, Operations, Finance, Storage Managers',
            'route' => '/departments',
            'summary' => 'Departments and canonical units make usage/refill reports meaningful without mixing incompatible measurements.',
            'features' => [
                'Create, archive, and recover managed departments; existing users begin under Unassigned.',
                'Assign a department from the user form and keep direct-manager routing separate.',
                'Movement history snapshots the employee department and manager at submission time.',
                'Reports filter by department, employee, manager, storage, item, reason, unit, package, and date.',
                'Exports show entered/package amount beside converted canonical quantity and protected proof links.',
                'Dashboard totals are grouped by compatible canonical unit instead of adding rolls, mL, grams, and metres together.',
            ],
            'steps' => [
                'Create departments and assign employees from Admins.',
                'Configure each item dimension, canonical unit, package presets, and proof policy.',
                'Use Scan Center or Mobile to submit measured usage/refill.',
                'Open Reports and filter by department, employee, item, package, unit, reason, storage, or date.',
            ],
            'rules' => [
                'One item has one canonical stock unit.',
                'Only admins define package conversion rates.',
                'Changing an employee department later does not rewrite old movement history.',
                'Requiring department assignment is configurable and disabled by default.',
            ],
        ],
        [
            'slug' => 'login-security',
