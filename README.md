...
Routes still live in `index.php`. Domain functions are loaded through `app/modules.php` and organized under `app/modules/`. Bootstrap-safe shared support code, such as permission catalogs, role defaults, request/security helpers, branding/upload options, and presentation helpers, lives under `app/support/`.
